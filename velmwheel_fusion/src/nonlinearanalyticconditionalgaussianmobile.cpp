...
namespace BFL
{
  ...
  ColumnVector NonLinearAnalyticConditionalGaussianMobile::ExpectedValueGet() const
  {
    ...
    state(1) += cos(state(3)) * vel(1) ;// + sin(state(3)) * vel(2);
    state(2) += sin(state(3)) * vel(1);// + cos(state(3)) * vel(2);
    state(3) += vel(3);
  ...
  Matrix NonLinearAnalyticConditionalGaussianMobile::dfGet(unsigned int i) const
  {
	...
	df(1,1)=1;
	df(1,2)=0;
	df(1,3)=-sin(state(3)) * vel(1);//+ cos(state(3)) * vel(2);
	df(2,1)=0;
	df(2,2)=1;
	df(2,3)=cos(state(3)) * vel(1);//- sin(state(3)) * vel(2);
	df(3,1)=0;
	df(3,2)=0;
	df(3,3)=1;
	return df;
