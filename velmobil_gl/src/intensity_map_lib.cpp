...
bool intensity_map::load_map(const std::string &file_path, std::vector<Eigen::Vector2f> &map_markers, size_t &used_markers_count)
{
    ...
    {
    	tmp_marker << v.second.get<float>("x"), v.second.get<float>("y");
    	map_markers.at(used_markers_count) = tmp_marker;
    	++used_markers_count;
    }
    return true;
}
bool intensity_map::save_map(const std::string &file_path, const std::vector<Eigen::Vector2f> &map_markers, size_t &used_markers_count)
{
...
bool intensity_map::getMarkers(std::vector<Eigen::Vector2f> &markers)
{
	markers = map_markers;
	return true;
}