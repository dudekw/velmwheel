...
double wfl_speed;
double wfr_speed;
long long int msg_wfl_enc_;
long long int msg_wfr_enc_;
long long int msg_wrr_enc_;
long long int msg_wrl_enc_;

long long int wrl_enc_old;
long long int wrr_enc_old;
long long int wfl_enc_old;
long long int wfr_enc_old;
int wrl_dist_diff, wrr_dist_diff, wfl_dist_diff, wfr_dist_diff;
...
std::array<double, 9> orientation_covariance = {0, 0, 0,
								   0, 0, 0,
								   0, 0, 0};
std::array<double, 9>  angular_velocity_covariance[9] = {0, 0, 0,
								   0, 0, 0,
								   0, 0, 0};
std::array<double, 9>  linear_acceleration_covariance[9] = {0, 0, 0,
								   0, 0, 0,
								   0, 0, 0};
...
bool VelmWheelCore::startHook() 
{
	...
	long long int wrl_enc_old = 0;
	long long int wrr_enc_old = 0;
	long long int wfl_enc_old = 0;
	long long int wfr_enc_old = 0;
...
void getWheelsSpeed()
{
	wheels_speed_enc[0] = wrl_dist_diff / (nsec.count() - nsec_old.count());
	wheels_speed_enc[1] = wfl_dist_diff / (nsec.count() - nsec_old.count());
	wheels_speed_enc[2] = wfr_dist_diff / (nsec.count() - nsec_old.count());
	wheels_speed_enc[3] = wrr_dist_diff / (nsec.count() - nsec_old.count());
}
...
void VelmWheelCore::updateHook() 
{
	...
	if (RTT::NewData == in_wrl_enc_.read(msg_wrl_enc_))
	{
		...
		sequence_enc += 1;
		wrl_dist_diff = ( (msg_wrl_enc_ - wrl_enc_old) / (50*5000) ) * ( (2 * 3.1415 * r) / 360 ); 
